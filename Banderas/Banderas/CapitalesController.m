...
@implementation CapitalesController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    //Declaramos donde se van a situar los mensajes de acierto/fallo
    _pointMensaje.x = 190.0;
    _pointMensaje.y = 275.0;
...
-(void)calcularBotones{
    ...
    [_button1 setTitle:country1.capital forState:UIControlStateNormal];
    [_button2 setTitle:country2.capital forState:UIControlStateNormal];
    [_button3 setTitle:country3.capital forState:UIControlStateNormal];
    [_button4 setTitle:country4.capital forState:UIControlStateNormal];
    
    //Aleatoriamente forzamos a que un boton sea la solucion
    int numSolucion = arc4random_uniform(4)+1;
    switch (numSolucion) {
        case 1:
            [_button1 setTitle:_capital forState:UIControlStateNormal];
            break;
        case 2:
            [_button2 setTitle:_capital forState:UIControlStateNormal];
            break;
        case 3:
            [_button3 setTitle:_capital forState:UIControlStateNormal];
            break;
        case 4:
            [_button4 setTitle:_capital forState:UIControlStateNormal];
            break;
    }
}
...
-(Country *)randomCountry{
    ...
    while(ok){
        ...
        country = [country transformFromDictionary:dic];
        if(![country.capital isEqualToString:_capital]){
            ok = false;
        }
    }
    return country;
...
-(void)calcularJuego{
    ...
    if(level == tamanio){
        ...
        if(_aciertos>_fallos){
            if(_fallos==0){
                mensaje = @"¡ENHORABUENA!, ha acertado todas las capitales";
            }else{
...
- (IBAction)action1:(id)sender {
    ...
    if([name1 isEqualToString:_capital]){
        [self showAcierto:_pointMensaje];
        _aciertos++;
    }else{
        [self showFallo:_pointMensaje texto:_capital];
        _fallos++;
    }
    //Recalculamos el juego
    [self calcularJuego];
...
- (IBAction)action2:(id)sender {
    ...
    if([name2 isEqualToString:_capital]){
        [self showAcierto:_pointMensaje];
        _aciertos++;
    }else{
        [self showFallo:_pointMensaje texto:_capital];
...
- (IBAction)action3:(id)sender {
    ...
    if([name3 isEqualToString:_capital]){
        [self showAcierto:_pointMensaje];
        _aciertos++;
    }else{
        [self showFallo:_pointMensaje texto:_capital];
...
- (IBAction)action4:(id)sender {
    ...
    if([name4 isEqualToString:_capital]){
        [self showAcierto:_pointMensaje];
        _aciertos++;
    }else{
        [self showFallo:_pointMensaje texto:_capital];
...
- (IBAction)showPais:(id)sender {
    [self showMessage:_pais atPoint:_pointMensaje];
}
